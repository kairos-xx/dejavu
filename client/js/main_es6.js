...
class Fmt {
    ...
    static timestamp(ms) {
        if (!ms) return "—";
        const d = new Date(ms);
        return `${Fmt.time(d)} · ${Fmt.pad2(d.getDate())}/${Fmt.pad2(d.getMonth() + 1)}`;
    }

    /** Compact relative age ("just now", "5 min ago", "2 days ago"). */
    static relative(ms) {
        if (!ms) return "";
        ...
        if (deltaMonth < 12) {
            return deltaMonth +
                (deltaMonth === 1 ? " month ago" : " months ago");
        }
        return `${Math.round(deltaMonth / 12)} yr ago`;
    }

    /** Signed size difference between two snapshots, e.g. "+12 KB". */
...
// Dev aid: when true, host.jsx is re-loaded via $.evalFile every
// time the CEP panel opens, so host-side edits take effect on a simple
// panel reopen. UXP still uses host/host.js directly from index.html.
const FORCE_HOST_RELOAD = true;
...
const LOCKED_FOLDER_TOKENS = ["$defaultFolder", "$filename"];
...
class SettingsStore {
    load() {
        try {
            ...
            // Only adopt keys that are part of the current settings
            // schema, dropping stale/unknown keys from older data.
