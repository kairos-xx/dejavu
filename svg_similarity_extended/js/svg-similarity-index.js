(function (global) {
  ...
  SVGSimilarityIndex.prototype.findSimilarToCurrentIllustratorDocument = function findSimilarToCurrentIllustratorDocument(folderOrEntry, options) {
    var self = this;
    return this.exportCurrentDocumentSVGWithMeta(options || {}).then(function (payload) {
      var targetOptions = {};
      var key;
      for (key in (options || {})) targetOptions[key] = options[key];
  ...
  SVGSimilarityIndex.prototype.findSimilarToFingerprint = function findSimilarToFingerprint(target, folderOrEntry, options) {
    ...
    return this.loadCache(folderOrEntry).then(function () {
      return self.adapter.listFiles(folderOrEntry, { recursive: o.recursive != null ? o.recursive : cfg.recursive !== false });
    }).then(function (files) {
      self.progress({ stage: "listed", total: files.length });
  ...
  SVGSimilarityIndex.prototype.indexFiles = function indexFiles(files, folderOrEntry) {
    ...
    var tasks = files.map(function (file) {
      if (file.size && file.size > max) return Promise.resolve(null);
